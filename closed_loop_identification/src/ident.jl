...
# --- 3. Dynamics ---
u0 = zeros(6)
u0[1] = deg2rad(10.0)  # Initial roll angle
function quadrotor_dynamics!(du, u, p, t)
    phi, theta = u[1], u[2]
    omega = u[4:6]

    throttles_function = p
    throttles = throttles_function(t)
    ...
    # Derivatives
    du[1:3] = W * omega
    du[4:6] = P.J \ (torques - 0.05 * omega - cross(omega, P.J * omega))
end
...
# --- 5. Visualization ---
# Main state plot
y = stack(map(u -> (rad2deg.([u[1], u[4]])), sol.u))'
